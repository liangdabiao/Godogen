...
### Animated sprite — Sprite Sheets

For character animations (run, attack, jump, idle, etc.), **sprite sheets are preferred** over the video-based workflow. A sprite sheet generates all frames in a single API call as an NxN grid, producing consistent, loopable animations without the complexity of video extraction and loop detection.

Read `${CLAUDE_SKILL_DIR}/sprite-gen.md` for full guide: animation types, prompting, Godot integration.

**CLI:**

```bash
# Generate a 4x4 run animation sprite sheet (5¢)
python3 ${CLAUDE_SKILL_DIR}/tools/sprite_gen.py spritesheet \
  --prompt "Sun Wukong, golden armor, monkey king, bold outlines, flat colors" \
  --animation run \
  --grid 4 \
  --rembg --split \
  -o assets/img/player_run.png
```

`--animation`: `auto`, `idle`, `walk`, `run`, `attack`, `cast`, `jump`, `dance`, `death`, `dodge`
`--grid`: `2` (4 frames), `3` (9 frames), `4` (16 frames, recommended)
`--rembg`: remove background via 302.ai recraft API + alpha binarization
`--split`: split into individual frame PNGs into `{stem}_frames/` directory
`--image`: optional reference image for edit-based generation (better consistency across animations)

**Output:** JSON to stdout: `{"ok": true, "path": "assets/img/player_run.png", "frames_dir": "assets/img/player_run_frames/", "cost_cents": 5, "grid": 4, "animation": "run", "frames": 16}`

**Cost:** ~5¢ per animation (nano-banana-2). With `--rembg`: ~6¢. With `--rembg --split`: ~6¢ + no extra cost.

**Multiple animations for same character:** generate each animation separately with the same `--prompt` description. Optionally use `--image` with the first generated sprite sheet as reference for better consistency across animations.

extends SceneTree
## Scene builder — run: timeout 60 godot --headless --script scenes/build_projectile.gd

func _initialize() -> void:
	print("Generating: QigongProjectile")

	var root := Area2D.new()
	root.name = "QigongProjectile"
	root.collision_layer = 32   # projectiles
	root.collision_mask = 8     # hazards (enemies)

	var script = load("res://scripts/qigong_projectile.gd")
	...
	if err != OK:
		...
		quit(1)
		return
	if not validate_packed_scene(packed, count, "res://scenes/projectile.tscn"):
		quit(1)
		return
	err = ResourceSaver.save(packed, "res://scenes/projectile.tscn")
	if err != OK:
